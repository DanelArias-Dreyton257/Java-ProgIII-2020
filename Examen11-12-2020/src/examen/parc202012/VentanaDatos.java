...
public class VentanaDatos extends JInternalFrame {
	 ...
	 * @param runnable Objeto runnable con código a ejecutar (run()) cuando el botón
	 *                 se pulse
	 * @param enHilo   Si es true, se lanza en un hilo independiente, si es false se
	 *                 lanza con el thread de Swing
	 */
	...
	public VentanaGeneral getVentMadre() {
		return ventMadre;
	}
	...
	public VentanaDatos(VentanaGeneral ventMadre, String tipo, String titulo) {
		...
		tDatos.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				if (e.getClickCount() >= 2) {
					...
					if (dobleClick != null)
						dobleClick.evento(fila, columna);
				}
			}
		});
		tDatos.addMouseMotionListener(new MouseMotionAdapter() {
			...
			public void mouseMoved(MouseEvent e) {
				...
				if (fila >= 0 && columna >= 0) {
					...
					if (valor != null && ventMadre != null)
						ventMadre.setMensajeSinCambioColor(valor.toString());
				}
			}
		});
		tDatos.getTableHeader().addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				if (e.getClickCount() >= 2) {
					int columna = tDatos.columnAtPoint(e.getPoint());
					if (dobleClickHeader != null)
						dobleClickHeader.evento(-1, columna);
				}
			}
		});
		tDatos.getTableHeader().addMouseMotionListener(new MouseMotionAdapter() {
			...
			public void mouseMoved(MouseEvent e) {
				int columna = tDatos.columnAtPoint(e.getPoint());
				if (columna >= 0) {
					Object valor = tDatos.getTableHeader().getColumnModel().getColumn(columna).getHeaderValue()
							.toString();
					if (valor != null && ventMadre != null)
						ventMadre.setMensajeSinCambioColor(valor.toString());
				}
		...
		tDatos.getColumnModel().getSelectionModel().addListSelectionListener(new ListSelectionListener() { // Salta en
																											// el cambio
																											// de
																											// columna
			@Override
		...
		tDatos.addKeyListener(new KeyAdapter() {
			boolean ultimaBusquedaEnCol = false; // Indica si la última búsqueda ha sido en columna (true) o en tabla
													// (false)
			...
			public void keyPressed(KeyEvent e) {
				...
				} else if (e.getKeyCode() == KeyEvent.VK_F && e.isControlDown()) { // Ctrl+F = Buscar en la columna
																					// actual
					ultimaBusqueda = JOptionPane.showInputDialog(VentanaDatos.this,
							"Texto a buscar en la columna actual:", ultimaBusqueda);
					ultimaBusquedaEnCol = true;
					hacerBusqueda();
				} else if (e.getKeyCode() == KeyEvent.VK_B && e.isControlDown()) { // Ctrl+B = Buscar en toda la tabla
																					// (primero hacia abajo luego hacia
																					// la derecha)
					ultimaBusqueda = JOptionPane.showInputDialog(VentanaDatos.this,
							"Texto a buscar en la tabla desde la posición actual:", ultimaBusqueda);
					ultimaBusquedaEnCol = false;
					hacerBusqueda();
				} else if (e.getKeyCode() == KeyEvent.VK_K && e.isControlDown()) { // Ctrl+K = Buscar de nuevo (repetir
																					// la última búsqueda)
					hacerBusqueda();
				} else if (e.getKeyCode() == KeyEvent.VK_PLUS && e.isControlDown()) { // Ctrl+K = Buscar de nuevo
																						// (repetir la última búsqueda)
					Font f = tDatos.getFont();
					tDatos.setFont(new Font(f.getFontName(), f.getStyle(), f.getSize() + 1));
					tDatos.repaint();
				} else if (e.getKeyCode() == KeyEvent.VK_MINUS && e.isControlDown()) { // Ctrl+K = Buscar de nuevo
																						// (repetir la última búsqueda)
					Font f = tDatos.getFont();
					tDatos.setFont(new Font(f.getFontName(), f.getStyle(), f.getSize() - 1));
					tDatos.repaint();
			...
			public void keyReleased(KeyEvent e) {
				if (e.isControlDown() && e.getKeyCode() == KeyEvent.VK_DOWN) { // Ctrl+cursor abajo - busca siguiente
																				// dato vacío si está en uno no vacío, y
																				// viceversa
				...
				} else if (e.isControlDown() && e.getKeyCode() == KeyEvent.VK_UP) { // Ctrl+cursor arriba - busca
																					// anterior dato vacío si está en
																					// uno no vacío, y viceversa
					// Buscar siguiente dato vacío de la columna actual
					int colActual = tDatos.getSelectedColumn();
					int row = tDatos.getSelectedRow();
					if (colActual != -1 && row > 0) {
						...
						while (row >= 0) {
							if ((!estoyEnVacio && tDatos.getValueAt(row, colActual).toString().isEmpty())
									|| (estoyEnVacio && !tDatos.getValueAt(row, colActual).toString().isEmpty())) {
								hacerScrollHastaFila(row, colActual, false); // Seleccionar fila encontrada en la
																				// ventana
								return;
							}
							row--;
			...
			private void hacerScrollHastaFila(int row, int col, boolean haciaAbajo) { // Mostrar en el scrollpane la
																						// fila,col indicada
				...
				if (haciaAbajo)
					...
					rect.setLocation(rect.x, rect.y - 25); // Y un poquito más arriba (para que no quede demasiado
															// justo)
		...
		/**
		 * T4 Extra (hay un fallo pero no se porque)
		 */
		tDatos.setDefaultRenderer(Object.class, new DefaultTableCellRenderer() {
			...
			public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected,
				...
				if (DatosDePrueba.grupo1 != null && DatosDePrueba.grupo2 != null && column==0)
				{
					
					boolean estaEnGrupo1 = DatosDePrueba.estaEnGrupo(tDatos.getValueAt(row, column).toString(), true);
					boolean estaEnGrupo2 = DatosDePrueba.estaEnGrupo(tDatos.getValueAt(row, column).toString(), false);

					if (estaEnGrupo1) {
						comp.setBackground(Color.ORANGE);
					} else if (estaEnGrupo2) {
		...
		// Cierre
		// setLocationRelativeTo( null ); // Centra la ventana en el escritorio (solo se
		// puede con JFrame)
		/*
		 * Renderer posible tDatos.setDefaultRenderer( Object.class, new
		 * DefaultTableCellRenderer() { private JProgressBar pb = new JProgressBar( 0,
		 * 500 ); private JLabel lVacia = new JLabel( "" ); private JLabel lError = new
		 * JLabel( "ERROR" ); public Component getTableCellRendererComponent(JTable
		 * table, Object value, boolean isSelected, boolean hasFocus, int row, int
		 * column) { if (column==2 || column==3) { try { if (value.toString().isEmpty())
		 * return lVacia; double val = Double.parseDouble( value.toString() );
		 * pb.setValue( (int) val*100 ); return pb; } catch (Exception e) {} return new
		 * JLabel( "Error" ); } else { Component comp =
		 * super.getTableCellRendererComponent( table, value, isSelected, hasFocus, row,
		 * column ); if (value instanceof String) { String string = (String) value;
		 * comp.setBackground( Color.white ); comp.setForeground( Color.black );
		 * ((JLabel)comp).setHorizontalAlignment( JLabel.LEFT ); if ("0".equals(string))
		 * { comp.setForeground( Color.LIGHT_GRAY );
		 * ((JLabel)comp).setHorizontalAlignment( JLabel.CENTER ); } if (column==1 ||
		 * column>=4 && column<=9) { ((JLabel)comp).setHorizontalAlignment(
		 * JLabel.CENTER ); } } return comp; } } } ); tDatos.addMouseMotionListener( new
		 * MouseMotionAdapter() {
		 * 
		 * @Override public void mouseMoved(MouseEvent e) { int fila =
		 * tDatos.rowAtPoint( e.getPoint() ); int columna = tDatos.columnAtPoint(
		 * e.getPoint() ); if (columna==2 || columna==3) { if
		 * (tDatos.getModel().getValueAt( fila, columna ).toString().isEmpty()) {
		 * lMensaje.setText( " " ); } else { lMensaje.setText( "Valor de satisfacción: "
		 * + tDatos.getModel().getValueAt( fila, columna ) ); } } else {
		 * lMensaje.setText( " " ); } } }); tDatos.addMouseListener( new MouseAdapter()
		 * {
		 * 
		 * @Override public void mouseClicked(MouseEvent e) { if (e.getClickCount()>=2)
		 * { // Doble click int fila = tDatos.rowAtPoint( e.getPoint() ); int columna =
		 * tDatos.columnAtPoint( e.getPoint() ); if (columna==0) { String centro =
		 * tDatos.getModel().getValueAt( fila, columna ).toString(); CentroEd centroEd =
		 * Datos.centros.get( centro ); JOptionPane.showMessageDialog(
		 * VentanaDatos.this, "Centro " + centro + "\nAbreviaturas: " +
		 * centroEd.getlAbrevs(), "Información de centro",
		 * JOptionPane.INFORMATION_MESSAGE ); centroEd.getlAbrevs(); } else {
		 * lMensaje.setText( " " ); } } } });
		 */
	}
	...
	public int getColumnWithHeader(String nomCol, boolean nomExacto) {
		...
		for (int col = 0; col < cols.getColumnCount(); col++) {
			...
			if (nomExacto && nom.equals(nomCol))
				return col;
			if (!nomExacto && nom.toUpperCase().startsWith(nomCol.toUpperCase()))
				return col;
		}
		return -1;
	}
