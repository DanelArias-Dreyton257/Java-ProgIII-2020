...
public class FarmaciaGuardia implements Serializable, Comparable<FarmaciaGuardia> {
	 ...
	 * @param horaDesde Hora de apertura de la farmacia (milisegundos desde las
	 *                  00:00 AM)
	 * @param horaHasta Hora de cierre de la farmacia (milisegundos desde las 00:00
	 *                  AM)
	 ...
	 */
	public FarmaciaGuardia(String localidad, long horaDesde, long horaHasta, String zona, String direccion,
			String telefono) {
		super();
	 ...
	 * 
	 * @param localidad       Localidad de la farmacia
	 * @param horario         Horario en formato "hh:mm - hh:mm" (primero la
	...
	public FarmaciaGuardia(String localidad, String horario, String zonaDirTelefono) {
		...
		if (barra > 0) {
			...
			if (telefono.toUpperCase().startsWith("TFNO: "))
				telefono = telefono.substring(6).trim();
	 ...
	 * 
	 * @return true si la farmacia tiene datos de hora de apertura y cierre,
	 *         localidad y dirección. false en caso contrario
	 */
	 ...
	 * 
	 * @param horaDesde Msgs transcurridos desde las 00:00 AM (se ignoran los
	 *                  días-fecha)
	 */
	...
	/**
	 * Indica si la farmacia está actualmente abierta o va a estar abierta en los
	 * siguientes minutos
	 * 
	 * @param proxMinutos Número de minutos a considerar (positivo o cero)
	 * @return true si está abierta ahora, o estará abierta en los minutos
	 *         indicados. false en caso contrario
	 */
	public boolean estaAbiertaAhora(int proxMinutos) {
		...
		horaAhora = (horaAhora + VentanaFarmacias.horaTest * 3600000L + VentanaFarmacias.minsTest * 60000L) % MSGS_DIA; // Modificación
																														// de
																														// hora
																														// actual
																														// para
	...
	public boolean equals(Object obj) {
		if (obj instanceof FarmaciaGuardia) {
			...
			return localidad != null && localidad.equals(f2.localidad) && horaDesde == f2.horaDesde
					&& horaHasta == f2.horaHasta && direccion != null && direccion.equals(f2.direccion);
		}
		return false;
	}
	...
	public int compareTo(FarmaciaGuardia o) {
		if (horaDesde != o.horaDesde)
			return (int) (horaDesde - o.horaDesde);
		if (horaHasta != o.horaHasta)
			return (int) (horaHasta - o.horaHasta);
		return (zona + direccion).compareTo(o.zona + o.direccion);
	}

	/**
	 * Devuelve la propiedad del teléfono de la farmacia de ser capicúa. Para ello
	 * se comparan los dígitos inicial y final y se devuelve su diferencia, sumada a
	 * la diferencia de los dígitos siguiente y anterior, y así sucesivamente.
	 * Cuando un número es capicúa, su diferencia total será cero. Si es cercano al
	 * capicúa, será un número pequeño. Por ejemplo: telefono = "94 4139000" -->
	 * calcCapicua() grande telefono = "94 413 14 49" --> calcCapicua() = 0 telefono
	 * = "944 132 439" --> calcCapicua() = 2
	 * 
	 * @return Cálculo de propiedad de capicua del teléfono de la farmacia (0 si es
	 *         perfectamente capicúa)
	 */
	...
	public static String codificarDir(String s) {
		boolean par = s.length() % 2 ==0;
		int pos = Math.floorDiv(s.length(), 2);
		String izq = s.substring(0,pos);
		String der = s.substring(pos+1, s.length());
		String c;
		if (par) {
			der = s.charAt(pos)+der;
			c = "";
		}
		else {
			c = s.charAt(pos)+"";
	...
	private static String codificarDir(String acum, String izq,String der) {
		if (izq.length()>0 && der.length()>0) {
			char d = der.charAt(0);
			acum+=d+"";
			char i =izq.charAt(izq.length()-1);
			acum+=i+"";
			return codificarDir(acum, izq.substring(0, izq.length()-1), der.substring(1,der.length()));
		}
		else return acum;
	...
	public static void main(String[] args) {
		System.out.println(codificarDir("Autonomía, 41"));
	}
