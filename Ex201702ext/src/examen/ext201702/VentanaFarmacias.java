...
public class VentanaFarmacias extends JFrame {
	...
	private Counter ctListado = new Counter();
	private Counter ctCapicua = new Counter();
	private Counter ctReloj = new Counter();
	// TAREA 3
	...
	public VentanaFarmacias() {
		...
		addComponentListener(new ComponentAdapter() {
			...
			public void componentResized(ComponentEvent e) {
				int w = jtableFarmacias.getWidth();
				int unSexto = w / 6;
				if (unSexto < 80) {
					jtableFarmacias.getColumnModel().getColumn(2).setWidth(80);
				} else {
					jtableFarmacias.getColumnModel().getColumn(2).setWidth(unSexto);
				}
			}
		});
		// TAREA 3
		...
		// Escuchador general de teclado (de esta forma se escucha el teclado
		// independientemente del componente que tenga el foco)
	...
	private void cerrar() {
		if (Main.hiloCambioPantalla != null)
			Main.hiloCambioPantalla.parar();
	}
	...
	Runnable cargaFarmaciasAhora2 = new Runnable() {
		@Override
		public void run() {
	...
	/**
	 * Mueve el scroll del área de texto derecha de farmacias de guardia al
	 * principio
	 */
	...
	Runnable inicioFA = new Runnable() {
		@Override
		public void run() {
	...
	/**
	 * Mueve el scroll del área de texto derecha de farmacias de guardia 2 pixels
	 * hacia abajo hasta que llega al final, en cuyo caso vuelve al principio
	 */
	public void mueveFarmaciasAhora() {
		lanzaEnEDT(mueveFA2);
	}

	Runnable mueveFA2 = new Runnable() {
		@Override
		public void run() {
	...
	/**
	 * Inicializa el formato de la jtable para que las filas de población / horario
	 * se vean en color cyan y negrita
	 */
	public void initFormatoTabla() {
		jtableFarmacias.setDefaultRenderer(Object.class, new DefaultTableCellRenderer() {
			...
			public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected,
				...
				if ("".equals(modeloDatosFarmacias.getValueAt(row, 2))) { // Si la columna 2 está vacía poner formato
																			// cyan - negrita
	...
	/**
	 * Carga las farmacias del mapa en la JTable de la ventana y hace que solo se
	 * puedan editar las celdas de dirección, y sus modificaciones se reflejen en el
	 * mapa
	 * 
	 * @param mapa Mapa de farmacias a visualizar. Si es null se utiliza (si existe)
	 *             el último utilizado). Si no lo hay, no hace nada.
	 */
	public void cargaFarmaciasEnTabla(MapaFarmacias mapa) {
		if (mapa == null)
			mapa = mapaTodasFarmacias; // Memoriza y usa el mapa que se pasó la última vez
		...
		public void run() {
			modeloDatosFarmacias = new DefaultTableModel() {
				...
				public boolean isCellEditable(int row, int column) {
					if (column == 1 && !"".equals(modeloDatosFarmacias.getValueAt(row, 2))) { // Si la columna 2 no está
																								// vacía es que la 1 es
																								// dirección
						return true;
					}
					return false;
				...
				public void setValueAt(Object aValue, int row, int column) {
					...
					for (ArrayList<FarmaciaGuardia> l : mapaTodasFarmacias.getMapaFarmacias().values()) {
						for (FarmaciaGuardia f : l) {
							if (modeloDatosFarmacias.getValueAt(row, 2).equals(f.getTelefono())) { // El teléfono
																									// identifica a la
																									// farmacia
			...
			for (ArrayList<FarmaciaGuardia> l : mapaTodasFarmacias.getMapaFarmacias().values()) {
				long horario = -1; // Memorización de la hora para separar las filas de las farmacias con las
									// mismas horas
				for (FarmaciaGuardia f : l) {
					if (horario != f.getHoraDesde() * 100 + f.getHoraHasta()) { // Cabecera de farmacias de misma
																				// localidad - hora
						horario = f.getHoraDesde() * 100 + f.getHoraHasta();
						// modeloDatosFarmacias.addRow( new Object[] { "", "", "" } );
						modeloDatosFarmacias.addRow(
								new Object[] { f.getLocalidad(), f.getHoraDesdeSt() + " - " + f.getHoraHastaSt(), "" });
						// modeloDatosFarmacias.addRow( new Object[] { "", "", "" } );
					}
	...
	Runnable inicioFT = new Runnable() {
		@Override
		public void run() {
			sbVerticalAbiertas.setValue(0);
		}
	...
	public void actualizarReloj() {
		String sReloj = " " + FarmaciaGuardia.sdfHM
				.format(new Date(System.currentTimeMillis() + horaTest * 3600000L + minsTest * 60000L)) + " ";
		if ((System.currentTimeMillis() / 1000) % 2 == 0)
			sReloj = sReloj.replace(":", " "); // Cambia a segundos pares e impares los : por espacio para dar el
												// feedback visual de que hay cambio en el reloj
		lReloj.setText(sReloj + "     ");
	}

	// Lanza un código (Runnable), asegurándose que se ejecuta desde el hilo de
	// Swing
	...
	private void mensajitoclick() {
		taMensajes.setText("Listado: " + (BD.selectCont(BD.LISTADO) + ctListado.getCount()) + ", Capicua: "
				+ (BD.selectCont(BD.CAPICUA) + ctCapicua.getCount()) + ", Reloj: "
				+ (BD.selectCont(BD.CLICK) + ctReloj.getCount()));
	}
}
